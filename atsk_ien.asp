...
docstart "Ienâkoðâ tûrisma atskaite","y1.jpg"
%>
<center><font color="GREEN" size="5"><b>Inâkoðâ tûrisma atskaite</b></font><hr>
<%
headlinks
...
grupa = getparameter("incoming")

%>

<form name="forma" action="atsk_komp.asp" method="POST">
<table>
<tr>
...
if request.form("skatit.x") <> "" then
	...
	if request.form("periods") = "day" then qb = "SELECT str(DatePart(yyyy,datums)) + '/' + str(DatePart(mm,datums))+'/'+str(DatePart(dd,datums)) as dat,count(distinct pid) as sk, Sum(piet_saite.summaLVL) as smLVL,  Sum(piet_saite.summaUSD) as smUSD,  Sum(piet_saite.summaEUR) as smEUR "
	if request.form("periods") = "month" then qb = "SELECT str(DatePart(yyyy,datums))+'/'+str(DatePart(mm,datums)) as dat,count(distinct pid) as sk, Sum(piet_saite.summaLVL) as smLVL,  Sum(piet_saite.summaUSD) as smUSD,  Sum(piet_saite.summaEUR) as smEUR "
	if request.form("periods") = "year" then qb = "SELECT str(DatePart(yyyy,datums)) as dat,count(distinct pid) as sk, Sum(piet_saite.summaLVL) as smLVL,  Sum(piet_saite.summaUSD) as smUSD,  Sum(piet_saite.summaEUR) as smEUR "
	...
	if rec.recordcount <> 0 then %>
		...
		if periods = "day" then
			...
			if formateddate(rec("dat"),"ymd")=tek_dat then
				...
				<tr bgcolor="#fff1cc">
					...
					<td align="right"><b><%=curr3print2(rec("smLVL"),rec("smUSD"),rec("smEUR"))%></b></td>
				</td></tr>
				<% 
		...
		<tr bgcolor="#bbbbbb">
			...
			<td align="right"><b><%=curr3print2(kop_smLVL,kop_sumUSD,kop_sumEUR)%></b></td>
		</td></tr>
