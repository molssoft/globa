...
function changeDocument(i){
	...
	$("#dok_nr_label_"+i).html(dok_nr_label);
	$("#dok_der_dat_label_"+i).html(dok_der_dat_label);
			...
			<form class="form-signin col-md-12 " method="post" id="forma" action="?f=Documents">
				...
				?>
					<div class="row col-md-12 " <? if ($i%2 == 0){ ?>
						...
						<div class="col-sm-7" style="display:none" id="dok_ievade_<?=$i;?>">

													
							
							<div class="col-sm-6">
								<label><span id="dok_nr_label_<?=$i;?>" <?if ($i>0 && false){?>class="visible-xs"<?}?>></span></label>
								...
								<div id="pases_dati_<?=$i;?>" class="pases_dati_<?=$i;?>">
									<input name="paseNR_<?=$i;?>" class="form-control " value="<?=$dalibnieks['paseS'];?><?=$dalibnieks['paseNR'];?>"
										autofocus id="paseNR_<?=$i;?>" style="text-transform:uppercase" autocomplete="off">
								...
								<div  id="ID_kartes_dati_<?=$i;?>" class="ID_kartes_dati_<?=$i;?>">
									<input name="idNR_<?=$i;?>" class="form-control" value="<?=$dalibnieks['idS'].$dalibnieks['idNR'];?>"								
									 autofocus id="idNR_<?=$i;?>" style="text-transform:uppercase" autocomplete="off">
							...
							<div class="col-sm-6">
									...
									<label><span id="dok_der_dat_label_<?=$i;?>" <?if ($i>0 && false){?>class="visible-xs"<?}?>></span></label>
								
									<div class="pases_dati_<?=$i;?>">
										<input name="paseDERdat_<?=$i;?>" class="form-control datepicker" value="<?=$dalibnieks['paseDERdat'];?>"
											autofocus id="paseDERdat_<?=$i;?>" placeholder="dd.mm.GGGG" autocomplete="off">
									...
									<div class="ID_kartes_dati_<?=$i;?>">
										<input name="idDerDat_<?=$i;?>" class="form-control datepicker"  value="<?=$dalibnieks['idDerDat'];?>"								
											autofocus id="idDerDat_<?=$i;?>" placeholder="dd.mm.GGGG" autocomplete="off">
						...
						<? if (isset($_SESSION['reservation_'.$online_rez_id]['documents_step']) && $_SESSION['reservation_'.$online_rez_id]['documents_step']==1){?>
						<script>
