...
class Pieteikums {
	...
	function PlaceInRoom($rez,$did,$viesn_id){
		...
		// viesnicai piesaistîtâ piemaksa
		...
		if (!empty($old_saites_arr)){
			...
			$FieldsToSave = $this->db->GetPietSaiteEditFields();
			 ...
			 foreach ($old_saites_arr as $old_saites_vals){
				 ...
				 //ja ir lauki, ko saglabât un tâ nav tukðâ piet_saite ar vietsk, ko izveido pçc pieteikuma izvedoðanas
...
//		- $atlaide + $piemaksa;
	...
	//atgrieþ  lietotâjam maksâjamo un iemaksâto summu par dâvanu karti
	...
	//ATgrieþ nâkamo insertçjamo dâvanu kartes numuru (dk_numurs)
	function GetDkNum(){
	...
	function GetDkWhere($where_arr){
		...
		$qry = "SELECT pieteikums.*,orez.no_delete,orez.dk_attelota
				FROM pieteikums 
				...
				ORDER BY pieteikums.datums ASC";
		//echo "$qry<br>";
	...
	//atgrieþ visus pieteikumus ar konrkçto vietu veidu
	...
	//---------------------------------------
	// 19.12.2019 RT
	// pârbauda, vai dotajam pieteikums eksistç kâds orderis (vai ir iemaksas)
	//------------------------------------------
	...
	//---------------------------------------
	// 19.12.2019 RT
	// atgrieþ online_rez pçc pieteikuma ID
	//------------------------------------------
