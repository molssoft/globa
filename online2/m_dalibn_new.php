...
//echo $sql."<br>";				
	...
	function CanSave($vards,$uzvards,$pk1,$pk2,$id = 0) {
		...
		if (!$row){
			...
			if ($internets) return true;
				...
				if(strtoupper(trim($vards)) == strtoupper($row['vards']) && strtoupper(trim($uzvards)) == strtoupper($row['uzvards'])) return true;
				else{
					//print_r($row);
	...
	//	echo "<br><br>";
		...
		if ($this->CanSave($data['vards'],$data['uzvards'],$data['pk1'],$data['pk2'])){
			...
			//	echo "eksistç PK";
				 $dalibn_id = $row['ID'];
				//updeito visus laukus					 
				 $this->Update( $data, $dalibn_id) ;
			}
			else{
				//$qry = "INSERT INTO dalibn (vards,uzvards,pk1,pk2) VALUES (?, ?, ?, ?)";
				//$params = array($vards,$uzvards,$pk1,$pk2); 
			
				/*$values = array ('vards' => strtoupper($data['vards']),
									'uzvards'=> strtoupper($data['uzvards']),
									'pk1' => $datapk1,
									'pk2' => $pk2);*/
				//echo "neeksistç PK<br>";
			...
			if (!isset($error['pk1'])){
				...
				if (!DEBUG){
					if (!PkValid($data['pk1'].$data['pk2'])){
						//echo "pk-valid";
						$error['pk1'] = 'Personas kods ievadîts nepareizi'; 
					}
				}
			}
	...
	function ValidatePhones($phones,$country_codes =null ){
		...
		//sâkuma vienalga kas, bet beigâs lai ri vismaz 8 cipari pçc kârtas		
		//$re = '/.*[\d]{8,}/';
		$re = "/[^\\d\\s+]/"; 
		if (DEBUG) echo $re;
		if (empty($phones['home_number']) &&
				...
				$error['home_number'] = 'Ievadiet vismaz 1 saziòas tâlruni';
	...
	function GetAgeIdRes($id){
		...
		$age = $this->GetAgeId($id,$beigu_dat);
		return $age;
	}
