...
class TransUid {
	...
	function Save($rez_id, $user_id, $s_post) {
		...
		//dim $ssql, $trans_id, $pid, $summa;
		//dim $str_array;
		//dim $temp_uid;
		require_once("i_functions.php");
		$temp_uid = GUID();
		//$temp_uid = mid($temp_uid, 2, strlen($temp_uid) -2)'---$noņ$em $figuriekavas;
		...
		foreach ($s_post as $key=>$val){
			//if (substr($key,0,8)=="payment_") {
				...
				if ($summa != '0.00'){
					...
					/*$ssql = "INSERT INTO trans_uid(trans_id, rez_id, pid, summa) 
								VALUES('".$trans_id."',".$rez_id.",".$pid.",".$summa.")";
