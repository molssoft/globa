...
class Db {
	...
	var $empty_date;
	var $tester_profiles = array(5116,5585,95,7996,87,84);//Ruuta, M�ris, Arta, Liene, Madara, J�nis Bite
	...
	public function connectAccounting($dbname) {
		$serverName = "SER-1\SQL14"; //serverName\instanceName

		$connectionInfo = array(
			"Database" => $dbname,  // Database name
			"Uid" => "www_globa",            // Database username
			"PWD" => "234lkjsdf90!(#",             // Database password
			"TrustServerCertificate" => "true",
			"CharacterSet" => "UTF-8",
		);
	...
	public function connectGlobaUtf8() {
		$serverName = "SER-DB3\mssql2008"; //serverName\instanceName
	...
	/**
	 * sqlsrv extended parameter: bind as NVARCHAR (Unicode). Same idea as N'text' in T-SQL literals.
	 * Use for UTF-8 PHP strings targeting nvarchar columns.
	 *
	 * @param string $value
	 * @param int $maxLen NVARCHAR size (1–4000, or use driver max for larger)
	 * @return array
	 */
	function nvarcharParam($value, $maxLen = 4000) {
		return array($value, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_NVARCHAR($maxLen));
	}

	/**
	 * sqlsrv extended parameter: bind as VARCHAR (non-Unicode / code page). Use e.g. after _win() (Windows-1257).
	 *
	 * @param string $value
	 * @param int $maxLen VARCHAR size (1–8000)
	 * @return array
	 */
	function varcharParam($value, $maxLen = 8000) {
		return array($value, SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_VARCHAR($maxLen));
	}
...
//atgrie� detaliz�tu v�sturi
	...
	//checkbox v�rt�bu p�rv�r� integer v�rt�b�
