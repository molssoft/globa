...
dim conn
OpenConn
docstart "Èarteru parâdnieki","y1.jpg" %>
<center><font color="GREEN" size="5"><b>Èarteru parâdnieki</b></font><hr>
<%
headlinks 
lietotajs = Request.QueryString("lietotajs")
Set rP = conn.Execute("select * from parametri")
Set r = conn.Execute("select * from pieteikums where izveidoja = '"+lietotajs+"' and deleted = 0 and gid = " + CStr(rP("charter")) + " and datums > '1/1/2003' and (bilanceLVL<>0 or bilanceUSD<>0 or bilanceEUR<>0) and beigu_datums<'" + SQLDate(Now) + "' order by beigu_datums")
