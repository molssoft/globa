...
%>
<select name=valsts size=1 id="valsts" style="width:200px">
...
<tr>
 ...
 <td bgcolor="#fff1cc">
	...
	<% set rKurators = conn.execute("select vards as v,* from lietotaji where exists (select * from tiesibusaites where lietotajsid = lietotaji.id and tiesibasid=12)  order by dbo.fn_str_to_num(ltrim(vards))")
...
%>
  ...
  <tr bgcolor="#fff1cc" >
    ...
    <td><font color=<%=c%> size="3"><%=f("kods")%>
		<% if f("pdf")<>"" then%>
		<br>
		<font size=1><a href="https://www.impro.lv/pdf/<%=f("pdf")%>.pdf" target="_blank">Ceïojuma apraksts</a></font>
		<% end if%>
	</td>
