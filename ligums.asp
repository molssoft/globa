...
if request("accepted")=1 then
	...
	if not pieteikumi.eof then
		while not pieteikumi.eof
			...
			if (bazes_cena_required <> 0) then
				ssql = "SELECT * FROm piet_saite WHERE vietas_veids IN (SELECT id FROM vietu_veidi WHERE (tips='C' OR tips='Z1' OR tips='CH1') and gid="+cstr(gid)+") AND deleted=0 and pid="+cstr(pid)+""
				'rw ssql
	...
	if clng(ligums_id)>0 and pids<>"" and request("test_tic")<>"1" then
		...
		conn.Execute(ssql)
		'--29.03.2019 RT, saglabâ arî pie online rez lîguma nr
		ssql = "UPDATE online_rez SET ligums_id = "&ligums_id&" WHERE id is not null and id in (select distinct online_rez FROM pieteikums WHERE id in ("&pids&") and deleted=0)"
		conn.Execute(ssql)
