...
If request.querystring("i") = "" Then 
	id = -1 
	'meklç prçc profila id#, ja norâdîts
	if (getNum(Request.Querystring("profili_id"))<>0) then
	
		query = "SELECT * FROM profili WHERE id="+cstr(Request.Querystring("profili_id"))
		set res_profils= conn.execute(query)
		if (not res_profils.eof) then
			pk1 = res_profils("pk1")
			pk2 = res_profils("pk2")
			vards = res_profils("vards")
			uzvards = res_profils("uzvards")
			query = "SELECT * FROM dalibn WHERE pk1='"+cstr(pk1)+"' and pk2='"+cstr(pk2)+"' AND vards='"+cstr(vards)+"'  AND uzvards='"+cstr(uzvards)+"' "
			'rw query
			set res_dalibn = conn.execute(query)
			if (not res_dalibn.eof) then
				id = res_dalibn("ID")
			end if
		end if
	end if 
...
dfields = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
	"adrese pilseta indekss talrunisM talrunisMvalsts talrunisD talrunisDvalsts talrunisMob talrunisMobvalsts fax eadr nosaukums reg nmkods vaditajs kontaktieris piezimes dzimta rajons novads ids idnr "+_
	"visas"

dtypes = "text text text text text text text text text text text text text text text text text text text text text text text text text num num text"

dforms = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
	"adrese pilseta indekss talrunisM talrunisMvalsts talrunisD talrunisDvalsts talrunisMob talrunisMobvalsts fax eadr nosaukums reg nmkods vaditajs kontaktieris piezimes dzimta rajons novads id_s id_nr visas"
...
if request.form("labot.x") <> "" then
	...
	valid_error = ""
	can_save = true
	'12.08.2019 RT: Pârbauda, vai kâdam citam dalîbniekam jau nav ðâds personas kods!	
	'09.01.2020 - neïauj saglabât ar vienâdiem pers. kodiem	
	If cstr(Request.form("pk1")) <> "" And cstr(Request.form("pk2")) <> "" then
		set rCitsDalibn = conn.execute("SELECT id, vards, uzvards FROM dalibn WHERE deleted=0 AND pk1='"+cstr(Request.form("pk1"))+"' AND pk2='"+cstr(Request.form("pk2"))+"' AND id<>"+ CStr(id))
		if not rCitsDalibn.eof then
			while not rCitsDalibn.eof
				citsID = rCitsDalibn("id")
				valid_error = valid_error + "Dalîbnieks ar ðâdu personas kodu "+cstr(Request.form("pk1"))+"-"+cstr(Request.form("pk2"))+" <a href='dalibn.asp?i="+cstr(citsID)+"' target=_blank>("+cstr(rCitsDalibn("vards"))+" "+cstr(rCitsDalibn("uzvards"))+")</a> jau eksistç.<br>"
				can_save = false
				rCitsDalibn.movenext
			wend
		end if
	end if
	'----------------'
	...
	if (can_save) then
		...
		'--pases derîguma termiòa validâcija (nedrîkst ppârsniegt 10 gadus no ðî brîþa)
		dim pasederdat
		pasederdat = Request.form("pasederdat")
		if pasederdat <> "" then
		
		'TODO: jâatjauno pârbaude, tikai ar datumiem jâtiek galâ
		'if (CDate(pasederdat) > DateAdd("yyyy",10,Now())) then
			''valid_error = valid_error + "No 20.11.2017. ceïojumiem ârpus Latvijas <b>neder pases ar derîguma termiòu uz 50 gadiem</b> (izsniedza lîdz 19.11.2007.)<b> un pases ar ielîmçtu fotokartîti </b>(izsniedza lîdz 30.06.2002.).<br>"
		''end if
		
		end if
		'Tâlruòa validâcija, ja LV numurs
		dim talrunis
		dim kods
		'Mâjas tâlruòa validâcija
		talrunis = Trim(cstr(Request.form("talrunisM")))
		kods = Trim(cstr(Request.form("talrunisMValsts")))
		if talrunis <> "" then
			if (kods <> "") then
				if (kods = "371") then
					if isNumeric(talrunis) then
						if len(talrunis) <> 8 then
							valid_error = valid_error + "LV mâjas tâlruòa numuram jâsastâv no 8 cipariem<br>"
						end if
					else
						valid_error = valid_error + "Mâjas tâlruòa numurs nav norâdîts pareizi<br>"
					end if
				end if
			else
				valid_error = valid_error + "Nav norâdîts mâjas tâlruòa valsts kods<br>"
			end if
		end if
		
		'Darba tâlruòa validâcija
		talrunis = Trim(cstr(Request.form("talrunisD")))
		kods = Trim(cstr(Request.form("talrunisDValsts")))
		if talrunis <> "" then
			if (kods <> "") then
				if (kods = "371") then
					if isNumeric(talrunis) then
						if len(talrunis) <> 8 then
							valid_error = valid_error + "LV darba tâlruòa numuram jâsastâv no 8 cipariem<br>"
						end if
					else
						valid_error = valid_error + "Darba tâlruòa numurs nav norâdîts pareizi<br>"
					end if
				end if
			else
				valid_error = valid_error + "Nav norâdîts darba tâlruòa valsts kods<br>"
			end if
		end if
		
		'Mobilâ tâlruòa validâcija
		talrunis = Trim(cstr(Request.form("talrunisMob")))
		kods = Trim(cstr(Request.form("talrunisMobValsts")))
		if talrunis <> "" then
			if (kods <> "") then
				if (kods = "371") then
					if isNumeric(talrunis) then
						if len(talrunis) <> 8 then
							valid_error = valid_error + "LV mobilâ tâlruòa numuram jâsastâv no 8 cipariem<br>"
						end if
					else
						valid_error = valid_error + "Mobilâ tâlruòa numurs nav norâdîts pareizi<br>"
					end if
				end if
			else
				valid_error = valid_error + "Nav norâdîts mobilâ tâlruòa valsts kods<br>"
			end if
		end if
		...
		dim pk1, pk2,dzimsanas_datums_var,pk1_var
		...
		conn.execute("update dalibn set " + _ 
			" idderdat = " + InputSQLDate(request.form("id_derdat")) + _
			" ,dzimsanas_datums = " + InputSQLDate(request.form("dzimsanas_datums")) + _
			" ,pasederdat = " + InputSQLDate(request.form("pasederdat")) + _
			" where id = " + cstr(id))
			
		'response.write "update dalibn set idderdat = " + InputSQLDate(request.form("id_derdat")) + " where id = " + cstr(id)
		''response.end
		
		Dim new_vals
		set new_vals = CreateDict(id_query) 
		UpdateActionDetails old_vals,new_vals,"dalibn",id,""
			'response.end
		'LogUpdateActionNew "dalibn", id
		id = session("editid")	
		...
		If cstr(Request.form("pk1")) <> "" And cstr(Request.form("pk2")) <> "" then
			...
			if (pk1 <> "" and pk2 <> "") then
				...
				If Not profils.eof Then
					...
					id_query = "select * from profili where id = " + CStr(profils_id)
					dfields = "vards uzvards pases pasenr paseterm pk1 pk2 "+ _
						"adrese pilseta indekss talrunisM talrunisMvalsts talrunisD talrunisDvalsts  talrunisMob talrunisMobvalsts eadr dzimta rajons novads ids idnr idderdat "+_
						"dzimsanas_datums"

					dtypes = "text text text text text text text text text text text text text text text text text text text text text text text text text num num text text text"

					dforms = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
						"adrese pilseta indekss talrunisM talrunisMvalsts talrunisD talrunisDvalsts talrunisMob talrunisMobvalsts eadr dzimta rajons novads id_s id_nr id_derdat dzimsanas_datums"

					EditRecord conn,id_query,dfields,dforms
					'conn.execute("UPDATE profili SET pk1= '"+cstr(Request.form("pk1"))+"', pk2 = '"+cstr(Request.form("pk2"))+"' WHERE id="+cstr(profils("id"))+"")
					'response.end
				End if
			end if
		...
		if (cstr(dzimsanas_datums_var) = "") then
				...
				' Drukaa dzimsanas datumu peec personas koda pirmaas daljas
					if len(pk1_var) = 6 then
						...
						if (diena>0 and diena<=31 and men>0 and men<=12 and gads<=year(Now())) then
							...
							dzimsanas_datums_var = cstr(diena) + "-" + cstr(men) + "-" + (cstr(gads))
							'rw dzimsanas_datums_var
							
							conn.execute("UPDATE dalibn SET dzimsanas_datums= '"+SQLDate(dzimsanas_datums_var)+"' WHERE id="+cstr(id)+"")
		...
		'update tikai_single
		tikai_single = 0
		if request.form("tikai_single") = "on" then tikai_single = 1
		conn.execute("update dalibn set " + _
			" tikai_single = " +cstr(tikai_single) + _
			" where id = " + cstr(id))
		
		'09.01.2020 RT - dzimums obligâti jânorâda
		If cstr(Request.form("dzimta")) = "" then
		 valid_error = valid_error + "Nav norâdîts dalîbnieka dzimums<br>"
		end if
		'-----------------------------------------------'

		'ja nav izdevies izkalkulçt dzimðanas datumu no pk
		if (dzimsanas_datums_var = "") then
			valid_error = valid_error + "Nav norâdîts dzimðanas datums.<br>"
		end if
		
	end if '--if can_save
	
	if valid_error <> "" then
		session("dmessage") = session("dmessage")+"<font color = red><br>"+valid_error+"</font>"
	end if
	
	if can_save then session("dmessage") = session("dmessage") + message+"<br>Labojumi izdarîti veiksmîgi!" end if
...
if request.form("meklet.x") <> "" then
  ...
  conn.execute("insert into log ([date],sql) values (getdate(),'"+SQLText(query)+"')")
  ...
  if skaits = 0 then  
	...
	talrunisMvalsts_var = request.form("talrunisMvalsts")
	talrunisDvalsts_var = request.form("talrunisDvalsts")
	talrunisMobvalsts_var = request.form("talrunisMobvalsts")
	fax_var = request.form("fax")
	eadr_var = request.form("eadr")
	piezimes_var = request.form("piezimes") 'replace(request.form("piezimes"),"""","''")
	visas_var = request.form("visas") 'replace(request.form("piezimes"),"""","''")
...
if id<>"-1" then
 ...
 if session("nomainit_pid")<>"" then
  Response.Redirect "nomainit_pid.asp?did="+cstr(id)
	...
	LogActionNew "dalibn",id,"skatîja"

	set recordset = server.createobject("ADODB.Recordset")
	recordset.open "select * from dalibn where  id = " + CStr(id),conn,3,3
	recordset.movefirst 
	
	'sql = "select * from dalibn where  id = " + CStr(id)
	...
	if pasederdat_var <> "" then
		if (CDate(recordset("pasederdat")) > DateAdd("yyyy",10,Now())) then
			
			pase_error = "No 20.11.2017. ceïojumiem ârpus Latvijas <b>neder pases ar derîguma termiòu uz 50 gadiem</b> (izsniedza lîdz 19.11.2007.)<b>"
		end if
	end if
	...
	talrunisMvalsts_var = recordset("talrunisMvalsts")
	talrunisDvalsts_var = recordset("talrunisDvalsts")
	talrunisMobvalsts_var = recordset("talrunisMobvalsts")
	if (error_talrunis = 0) then
	
		if (talrunisM_var <> "") then	
			if len(trim(talrunisMvalsts_var)) = 0 or not cint(isNumeric(trim(talrunisMvalsts_var))) then
				talrunisMvalsts_var = "371"
			end if		
		end if
		
		if (talrunisD_var <> "") then	
			if len(trim(talrunisDvalsts_var)) =0  or not cint(isNumeric(trim(talrunisDvalsts_var))) then
				talrunisDvalsts_var = "371"
			end if		
		end if
		
		if (talrunisMob_var <> "") then	 
			if len(trim(talrunisMobvalsts_var)) =0  or not cint(isNumeric(trim(talrunisMobvalsts_var))) then
				talrunisMobvalsts_var = "371"
			end if		
		end if
	...
	piezimes_var =  recordset("piezimes") 'replace(recordset("piezimes"),"""","''")
	visas_var =  recordset("visas") 'replace(recordset("piezimes"),"""","''")
	...
	''piezîmes visiem no pçdçjâ pieteikuma
	set rpp = conn.execute("select top 1 piezimes_visiem from pieteikums where did = "+cstr(id) + " and deleted = 0 order by id desc")
	piezimes_visiem = ""
	if not rpp.eof then
		piezimes_visiem = rpp(0)
	end if
end if
...
<script language="javascript" type"text/javascript">
	...
	function allowRes(id,pk1,pk2){
		if ((id==pk1 &&  pk2=='X')){
			alert('Pirms pietiekuma pievienoðanas, lûdzu, atjaunojiet dalîbnieka datus!');
			return false;
		}
		else{
			forma.action='pieteikums.asp?command=new&did=<%=id%>&time=<%=cstr(hour(now))+cstr(minute(now))+cstr(second(now))%>';
...
if pase_error <> "" then
	%>
	<center><font color="red"><b><br><%=pase_error%></b></font><br></center>
	<%
end if
...
<% if id = -1 then 
'ja ir atrasti vairâki dalîbnieki pçc rezrevâcijas ID#
 if (Request.Querystring("mekl_rez_id")) then
	query = "SELECT * FROM dalibn WHERE id in (select did from pieteikums where online_rez="+cstr(Request.Querystring("mekl_rez_id"))+" AND deleted=0) OR id IN (SELECT did FROM piet_saite WHERE deleted=0 AND pid="+cstr(Request.Querystring("mekl_rez_id"))+")"
	session("dquery") = query
        session("dalibn_sar_mode") = "show"
	response.redirect "dalibn_sar.asp"
 end if
 %>
	...
	<div style="width:800px;text-align:left">
		<form action="c_dalibn.php" method="POST" id="form_meklet_pec_rez" style=""   >
			<table style="float:right"s><tr><td><b>Meklçt pçc rezervâcijas Nr.:</b></td><td><input type="text" placeholder="Rez. Nr." name="search_rez" value="<%=Request.Querystring("search_rez")%>" size="8" > </td><td><input type="image"  name="meklet_pec_rez" onclick="$('#form_meklet_pec_rez').submit()" title="Meklçt" src="impro/bildes/meklet.jpg"></td></tr></table>
		
		</form>
		<form action="c_dalibn.php" method="POST" id="form_meklet_pec_ID" style="float:right;padding-right:25px">
			<table style="float:right"><tr><td><b>Meklçt pçc dalîbnieka Nr.:</b></td><td><input type="text" placeholder="Dalîbnieka Nr." name="search_ID" value="<%=Request.Querystring("search_id")%>" size="8" > </td><td><input type="image"  name="meklet_pec_ID" onclick="$('#form_meklet_pec_ID').submit()" title="Meklçt" src="impro/bildes/meklet.jpg"></td></tr></table><br>
	...
	<div style="width:800px;text-align:left"><br>
	<%if (Request.Querystring("error_rez") = "not_found") then 
		%>
		<center><font color="red"><b><br>Rezervâcijai nav atrasts neviens dalîbnieks</b></font><br></center>
		<%
	end if%>
	<%if (Request.Querystring("error") = "not_found") then 
		%>
		<center><font color="red"><b><br>Dalîbnieks ar ðâdu Nr. nav atrasts</b></font><br></center>
		<%
...
<%else
	if (Request.Querystring("search_id") or  Request.Querystring("search_rez")) and Request.Querystring("i") then
			message = "Atrasts 1 dalîbnieks!"
...
<td align="right">Vârds:</td><td><input type="text" size="25" autocomplete="off" maxlength="128" name="vards" value="<%=vards_var%>" style="text-transform: uppercase" <%if isDalibnInBlockedGroup(id) then %> onchange="alert('Dalîbnieka Vârdu/Uzvârdu nevar mainît, jo dalîbnieks atrodas bloíçtâ grupâ. Lûdzu griezties pie kuratora.'); forma.vards.value='<%=vards_var%>';"<%end if%>></td>
<td align="right">Uzvârds:</td><td> <input type="text" size="25" autocomplete="off" maxlength="128" name="uzvards" value="<%=uzvards_var%>" style="text-transform: uppercase" <%if isDalibnInBlockedGroup(id) then %> onchange="alert('Dalîbnieka Vârdu/Uzvârdu nevar mainît, jo dalîbnieks atrodas bloíçtâ grupâ. Lûdzu griesties pie kuratora.'); forma.uzvards.value='<%=uzvards_var%>';"<%end if%>></td>
</tr>
<tr  bgcolor=NavajoWhite>
...
<tr bgcolor=NavajoWhite>
	...
	<td>
		<input type="text" size="2" maxlength="2" name="paseS"  style="text-transform:uppercase"  value="<%=paseS_var%>">
		<input type="text" size="7" maxlength="7" name="paseNR"  style="text-transform:uppercase"  value="<%=paseNR_var%>"> 
	</td>
...
<tr  bgcolor=NavajoWhite>
	...
	<td>
		<input type="text" size="2" maxlength="2" name="id_S"  style="text-transform:uppercase" value="<%=id_S_var%>">
		<input type="text" size="7" maxlength="7" name="id_NR"  style="text-transform:uppercase"  value="<%=id_NR_var%>"> 
	</td>
...
<tr bgcolor=NavajoWhite>
<td align="right">Mâjas t.: </td><td>+<input type="text" size="1" maxlength="4" name="talrunisMvalsts" value="<%=talrunisMvalsts_var%>"><input type="text" size="14" maxlength="15" name="talrunisM" value="<%=talrunisM_var%>"></td>
<td align="right"> Darba t.: </td><td>+<input type="text" size="1" maxlength="4" name="talrunisDvalsts" value="<%=talrunisDvalsts_var%>"><input type="text" size="14" maxlength="15" name="talrunisD" value="<%=talrunisD_var%>"></td>
<td align="right"> Mobilais t.: </td><td>+<input type="text" size="1" maxlength="4" name="talrunisMobvalsts" value="<%=talrunisMobvalsts_var%>"><input type="text" size="14" maxlength="15" name="talrunisMob" value="<%=talrunisMob_var%>"></td>
</tr>
<tr bgcolor=NavajoWhite>
...
<td colspan="2"><%=piezimes_visiem%>&nbsp;</td>
</tr>
<tr  bgcolor=NavajoWhite>
<td>Vîzas: </td><td  colspan=3><input type="text" size = "88"  name="visas"  value="<%=visas_var%>" /></td>

<td colspan="2">&nbsp;</td>

</tr>
<%
set rDal = conn.execute("select tikai_single from dalibn where id = "+cstr(id))
tikai_single = "  "
if not rDal.Eof then
	if rDal("tikai_single") = 1 then tikai_single = " checked "
end if
%>

<tr bgcolor=NavajoWhite>
	<td></td><td  colspan=3>Atïauts tikai vienvietîgais<input type=checkbox name=tikai_single <%=tikai_single%></td>
	<td colspan="2">&nbsp;</td>
</tr>
...
<BR><BR>
<input type="image" src="impro/bildes/dokumenti.jpg" name="dokumenti" onclick="forma.action='dokuments.php?did=<%=id%>';"><BR>
<% doc_count = conn.execute("select isnull(count(*),0) as c from dokumenti where did = " + cstr(id))(0)%>
[<%=doc_count%>]
...
<center>
<% 
if adrese_var <> "" and pilseta_var <> "" and not (adrese_var="X" and pilseta_var="X") then
...
If pk1_var <> "" And pk2_var <> "" then
	Set ronline = conn.execute("select id,banned,ban_date from profili where pk1 = '"+pk1_var+"' and pk2 = '"+pk2_var+"'")
	If Not ronline.eof Then
		Response.write "<BR>Izveidots online <a href=profils.asp?pk1="+pk1_var+"&pk2="+pk2_var+">profils</a><BR>"
		If ronline("banned") then
			%>
			<font size="3" color="red">Liegta pieeja tieðsaistes rezervâcijas sistçmai no <%=dateprint(ronline("ban_date"))%></font>
			<%
		end if
	End if
End if

''-------------------- vai ir grupas vadîtâjs
%><script>function NoAccountingPermisions() { alert("Nepiecieðamas grâmatveþa tiesîbas"); }</script><%
if IsAccess(T_GRAMATVEDIS) then
	Set rvad = conn.execute("select * from grupu_vaditaji where deleted = 0 order by replace(vards2,'#',''), uzvards2")
	%>Grupas vadîtâjs: 
	<select name=vad_id><%
		%><option value=0></option><%
		while not rvad.eof
			%><option value=<%=rvad("idnum")%><% if nullprint(rvad("did")) = nullprint(id) then response.write " selected " %>><%=rvad("vards")%>&nbsp;<%=rvad("uzvards")%></option><%
			rvad.movenext
		wend
	%></select><%
	%> <a href="#" onclick="
		TopSubmit('dalibn_save_grupas_vad.asp?id=<%=id%>');">[Saglabât]</a><BR><BR><%
else
	Set rvad = conn.execute("select * from grupu_vaditaji where idnum = " + cstr(id))
	if rvad.eof then	
		Response.write "<BR>Nav grupas vadîtâjs"
	else
		Response.write "<BR>Ir grupas vadîtâjs"
	end if
	%> <a href="#" onclick="NoAccountingPermisions();return false;">[Mainît]</a><BR><BR><%
end if
...
<hr>
<a href="dokuments.php?did=<%=id%>">Dokumenti</a>
<% set rDok = conn.execute("select * from dokumenti where did = " + cstr(id))
while not rDok.eof
	response.write "<BR>" + rDok("file_original_name")
	rDok.MoveNext
wend
%>
<hr>
...
oldpquery = "SELECT pieteikums.id as pid,pieteikums.datums,gid,old_gid,grupa.valuta,agents," + _
	...
	pquery="SELECT pieteikums.id as pid,pieteikums.datums,gid,old_gid,grupa.valuta,agents," + _
		...
		"pieteikums.izmaksasEUR,pieteikums.atlaidesLVL,pieteikums.atlaidesUSD,pieteikums.atlaidesEUR," + _
		"pieteikums.sadardzinLVL,pieteikums.sadardzinUSD,pieteikums.sadardzinEUR,pieteikums.bilanceLVL," + _
		"pieteikums.bilanceUSD,pieteikums.bilanceEUR,krasa,grupa.kods,v,grupa.sakuma_dat,grupa.beigu_dat,grupa.atcelta," + _
		"agents_izv,dk_norakstita, Sum(isnull(o.summaLVL,0)) as neapstLVL, Sum(isnull(o.summaUSD,0)) as neapstUSD," + _
		" Sum(isnull(o.summaEUR,0)) as neapstEUR, isnull(pieteikums.internets,0) as internets,ligums_id,online_rez " + _
		"FROM (pieteikums LEFT JOIN grupa ON pieteikums.gid = grupa.id) " + _
		"LEFT JOIN marsruts ON marsruts.id = grupa.mid " + _
		"LEFT JOIN orderis o ON o.pid = pieteikums.id and o.deleted = 0 and o.parbaude = 1 " + _
		"LEFT JOIN grupa old_grupa ON old_grupa.ID = old_gid " +_
		
		"WHERE (pieteikums.id in (select pid from piet_saite where did = "+cstr(id) + " and piet_saite.deleted = 0) OR pieteikums.id IN (SELECT ID FROM pieteikums WHERE did="+cstr(id) + " AND pieteikums.deleted=0)) " + _
		"and (pieteikums.deleted = 0 or pieteikums.atcelts = 1) and ((pieteikums.tmp = 0 " + _
		"and pieteikums.internets=1) or (isnull(pieteikums.internets,0)=0)) " + _
		...
		"case when isnull(grupa.sakuma_dat,"+sis_gads+")>="+sis_gads+" then -9999999999 else -99999999999 end end  end)) " +_
		" and pieteikums.datums >= case when marsruts.v like '!%' AND (marsruts.v not like '!Atteik%' and marsruts.v not like '%Atcelta%') then "+sak_datums+" else "+senakais_datums+" end " +_
		
		" and ((isnull(pieteikums.BilanceEUR,0) <> case when marsruts.v like '!%' AND (marsruts.v not like '!Atteik%' and marsruts.v not like '%Atcelta%') and pieteikums.datums<"+sis_gads+" and pieteikums.datums>="+sak_datums+" then 0 else case" +_
		" when marsruts.v like '!%' AND (marsruts.v not like '!Atteik%' and marsruts.v not like '%Atcelta%')  and pieteikums.datums<"+sak_datums+" then pieteikums.BilanceEUR else case" +_
		" when marsruts.v like '!%' AND (marsruts.v not like '!Atteik%'  and marsruts.v not like '%Atcelta%')   and pieteikums.datums>="+sis_gads+" then -999999999 else -9999999999 end end end))  " +_
		
		" and "+pernais_gads+"<= case when (marsruts.v like '!Atteik%' or marsruts.v like '!Atcelta%')		then old_grupa.sakuma_dat else "+pernais_gads+" end "+_
		" and "+senakais_datums+"<= case when (marsruts.v NOT like '!Atteik%'  and marsruts.v not like '%Atcelta%')  then pieteikums.datums else "+senakais_datums+" end "+_
	
		"group by pieteikums.id,pieteikums.datums,gid,old_gid,grupa.valuta,agents,pieteikums.summa,iemaksas,izmaksas,atlaides,bilance,atcelts," + _
		"pieteikums.piezimes,pieteikums.summaLVL,pieteikums.summaUSD,pieteikums.summaEUR,pieteikums.iemaksasLVL,pieteikums.iemaksasUSD," + _
		"pieteikums.iemaksasEUR,pieteikums.izmaksasLVL,pieteikums.izmaksasUSD,pieteikums.izmaksasEUR,pieteikums.atlaidesLVL,pieteikums.atlaidesUSD," + _
		"pieteikums.atlaidesEUR,pieteikums.sadardzinLVL,pieteikums.sadardzinUSD,pieteikums.sadardzinEUR,pieteikums.bilanceLVL,pieteikums.bilanceUSD," + _
		"pieteikums.bilanceEUR,krasa,grupa.kods,v,grupa.sakuma_dat,grupa.beigu_dat,grupa.atcelta,agents_izv,dk_norakstita,pieteikums.internets " + _
...
<th align = "center" style="min-width:90px;width:90px">Lîgums</th>
<th align = "center" style="min-width:90px;width:90px">Dok.</th>
<th align = "left"></th>
</tr>
<%
p = 0
while not pieteikumi.eof
''response.write pieteikumi("pid")
 ...
 if pieteikumi("pid")<>p then
	...
	if getnum(pieteikumi("old_gid"))<>0 then 
	 ...
	 response.write "<td"+fstyle+">"+nullprint(pieteikumi("kods"))+" " + nullprint(pieteikumi("v"))
		if (InStr(pieteikumi("v"),"!Dâvanu karte") = 0) then
			response.write  " " + dateprint(pieteikumi("sakuma_dat")) + " " + dateprint(pieteikumi("beigu_dat"))
		end if
		response.write piezimes	
	end if
	...
	''If gads>=2020   Then '''xxx
		''Set r = conn.execute("select * from [SER-1\SQL14].s"+CStr(gads)+".dbo.rekini_tr tr inner join [SER-1\SQL14].s"+CStr(gads)+".dbo.rekini r on tr.rekins=r.id where globa_pid = "+CStr(pieteikumi("pid")))
		''If Not r.eof Then
		''	  zz><a href=orderis.asp?pid=<zz=pieteikumi("pid")zz>&zils=1&rekins=<zz=r("num")zz>&rekins_gads=<zz=CStr(gads)zz>&kredits=2.3.4.X><zz
	...
	<%
		...
		if getnum(pieteikumi("online_rez"))<>0 then
			...
			if not rrez.eof then
				if (getnum(rrez("ligums_id")<>0)) then
					ligums_atrasts = true

					if (getNum(pieteikumi("iemaksasEUR"))<>0) then
		...
		if (not ligums_atrasts) then
		
			if getnum(pieteikumi("ligums_id"))<>0 then
			ssql = "SELECT isnull(nosutits,0) as nosutits,isnull(parakstits,0) as parakstits FROM ligumi WHERE id="+cstr(pieteikumi("ligums_id"))+""
			set lig = conn.execute(ssql)
			parakstits = lig("parakstits")
			nosutits = lig("nosutits")
	...
	<td>
	<% ''dokumenti 
		set rDocCount = conn.execute("select isnull(count(*),0) as c from dokumenti where pid = "+cstr(pieteikumi("pid")))
		if (rDocCount("c")<>0) then
			%><a href=dokuments.php?pid=<%=pieteikumi("pid")%>><%=rDocCount("c")%></a><%
		end if
	%>
	</td>
 ...
 if session("parsk") = "no" or session("parsk") = "uz" then
  if cstr(session("parsk_first")) <> cstr(pieteikumi("pid")) then %><td><input type = 'image' src='impro/bildes/parskaitit.jpg' name='parskaitit' alt = '' onclick=forma.action='operacija.asp?pid=<%=pieteikumi("pid")%>&did=<%=id%>'></td><%
	end if
end if
...
if id<>"-1" then 
	...
	<select name="gid">
	 <% set rParam = conn.execute ("select * from parametri") %>
	  ...
	  <option value="<%=rParam("konferences")%>">Konference</option>
	  ...
	  <option value="balle">Balles biïete</option>
		...
		If isaccess(T_LIETOT_ADMIN) Then
			%><BR><br><a href=melnais_saraksts.asp?id=<%=id%> onclick="return confirm('Vai tieðâm vçlaties ielikt personu melnajâ sarakstâ?');">Ielikt melnajâ sarakstâ</a><%
		End if
...
while field_l<>""
	if val_l <> "" then
		...
		if type_l = "string" or type_l = "text" then 
			' Split input by spaces
			arrWords = Split(Trim(val_l), " ")

			For xi = 0 To UBound(arrWords)
				If Trim(arrWords(xi)) <> "" Then
					result_l = result_l & " and " & field_l & " like '%" & Replace(Trim(arrWords(xi)), "'", "''") & "%'"
				End If
			Next
