...
title = "Vietu veidu tipi"
table = "vietu_veidi_tipi"
order = "nosaukums"
captions = "Nosaukums,kods"
captions_mas = split(captions,",")
fields = "nosaukums,kods"
...
if r.recordcount <> 0 then
   ...
   for i = 0 to last 
    if types_mas(i) = "varchar" then
 		  %><td><input type="text" name="<%=fields_mas(i)%><%=r("id")%>" size="<%=sizes_mas(i)%>" value="<%=NullPrint(r(fields_mas(i)))%>"></td><%
 		 elseif types_mas(i) = "money" then
