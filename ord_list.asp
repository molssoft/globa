...
if qc > 500 then 
	%></table>
	Atrastas vairâk nekâ 500 operâcijas. Nevar parâdît.<%
	response.end
end if
...
'response.write(nullprint(f("nopid")) +"<<<<< nopid <br>")
	...
	if(nullprint(f("pid")) <> "" and nullprint(f("pid"))<>0) then
		set dalibn = conn.execute("SELECT DISTINCT did FROm piet_saite WHERE pid="+cstr(f("pid"))+" AND did IS NOT NULL")
		if not dalibn.eof then
			...
			set dalibn = conn.execute("SELECT DISTINCT did FROm pieteikums WHERE id="+cstr(f("pid"))+"")
