...
ielid_laiks_uz = rGr("ielid_laiks_uz") '--- ielidoðanas laiks uz mçríi
ielid_laiks_no = rGr("ielid_laiks_no") '--- ielidoðanas laiks no mçría
sapulces_dat = rGr("sapulces_dat")
pdf = rGrupa("apraksts")
carter_viesn_id = rGrupa("c_vid") 'rGrupa("carter_viesn_id")!!!! nedarbojas. ja ieliek priekðâ izbr_vieta, tad nedarbojas izbr_vieta wtf?
carter = rGr("carter")
...
<table>
  ...
  <% if not isaccess(T_DROSIBA) then %>
		...
		<tr>
			<td align=right>
				<font size=2><strong>LV ceïojums, online nevajag norâdît pasi:</strong></font></td>
...
<td align=right><font size=2><strong>Èarteris: </strong></font></td>
    ...
    <span id="carter_viesn">
        ...
        <select id="carter_viesn_id" name="carter_viesn_id">
          ...
          <% 
          '--- vecâs èarterviesnîcas jaunâm grupâm slçpj
...
<tr>
    <td align=right><font size=2><strong>Viesnîcu numuri:</strong></td>
...
<tr>
    ...
    <td align=left>
        ...
        <tr>
            <td></td>
            <td><div id="standarta_viesnicas_message" style="width: 500px; display: none;">Ja vçlaties, lai pie standarta viesnîcâm tiktu izveidoti arî SINGLE numuriòi, lûdzu, pârliecinieties vai grupai ir pievienots pakalpojums ar tipu "Piemaksa SGL"! <a href='vietu_veidi.asp?gid=<%=gid%>' target="_blank"><font color="GREEN" size="3"><b>Pakalpojumi</b></font></a></div></td>
        </tr>
		<!--
		<tr>
			<td align=right><font size=2><strong>Sï¿½tï¿½t epastu par atteikumiem:</strong></td>
...
<tr>
    <td align=right><strong><font size=2>Drukât vauèerus:</font></strong></td>
...
<tr>
    <td align=right><font size=2><strong>1. termiòð:</strong></font></td>
    <td align=left>
...
<tr>
    ...
    <td>
        ...
        <a href="c_grupu_vad_grafiks.php?datums=<%=Year(rGrupa("sakuma_dat"))%>_<%=Month(rGrupa("sakuma_dat"))%>" target="_blank">Skatît grupas vadîtâju grafiku</a>
    </td>
</tr>
...
<% if mid(kods,4,1) = "V" or mid(kods,4,1) = "T" then %>
    <% set rGrupa = conn.execute ("select * from grupa where id = "+gid) %>
    <tr>
        <td align=right><font size=2><strong>Kontroliera meklçtâjs:</strong></td>
        <td>
            ...
            <input type="image" src="impro/bildes/diskete.jpg" alt="Saglabï¿½t izmaiï¿½as laukï¿½ - Kontroliera meklï¿½tï¿½js." onclick="javascript: TopSubmit('grupa_save.php?kontrolieris='+forma.kontrolieris.value)" name="save_kontrolieri">
        </td>
    </tr>
<% else %>
...
<tr>
    <td align=right><font size=2><strong>Meklçtâjam draudzîga saite:</strong></td>
    <td align=left><input type=text name=slug size=70 value="<%=rGrupa("slug")%>"></td>
</tr>

<tr>
    <td align=right><font size=2><strong>Vietu apraksti mâjas lapai:</strong></td>
    <td align=left>
        <select name="vietu_apraksts" id="vietu_apraksts">
            ...
            wend
            %>
        </select> <a href="c_vietu_apraksti.php" target="_blank">Skatît aprakstus</a>
		...
		<%
			...
			while not rAprPievienotie.eof
				...
				<a href="vietas_apraksts_delete.php?id=<%=rAprPievienotie("id")%>&gid=<%=gid%>" 
					onclick="return confirm('Vai tieï¿½ï¿½m vï¿½lies dzï¿½st ï¿½o ierakstu?');">[x]</a>
				<%
...
<td valign=top>
    ...
    <% end if %>
    <li><a href='viesnicas.asp?gid=<%=gid%>'><font color="GREEN" size="3"><b>Viesnîcu numuri</b></font></a></li>
    <li><a href='viesnicas_veidi.asp?gid=<%=gid%>'><b><font color="GREEN">Viesnîcu numuru veidi</b></font></a></li>
    ...
    <% end if %>
    <li><a href="c_bildes.php?gid=<%=gid%>"><font color="GREEN" size="3"><b>Biïeðu atslçgvârdi</b></a></li>

    <% if pdf<>"" then%>
    <br>
    <li><a href="https://www.impro.lv/pdf/<%=pdf%>.pdf" target="_blank"><font color="GREEN" size="3"><b>Ceïojuma apraksts</b></a></li>
    <% end if%>
    <li><a href="c_grupas_sar_soc.php?gid=<%=gid%>" target="_blank"><font color="GREEN" size="3"><b>Saraksts sociâlajiem tîkliem</b></a></li>
...
<a href="vesture.asp?tabula=grupa&id=<%=gid%>" target="vesture">
 <img style="border:0px" src="impro/bildes/clock.bmp" alt="Vçsture">
</a>
...
%>
 <a href="grupa_del.asp?gid=<%=gid%>">
  <img style="border:0px" onclick="return confirm('Dzçst grupu?');" src="impro/bildes/dzest.jpg" alt="Dzçst">
 </a>
<%
end if
%>
