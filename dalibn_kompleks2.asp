...
%><center><br><br><font size=5>Kopleksie klienti par <%=y%>. gadu</font><br><br><%
set r = conn.execute("select * from dalibn where adrese<>'' and deleted = 0 and id in (select did from piet_saite where deleted = 0 and pid in (select id from pieteikums where gid = 292 and beigu_datums >= '1/1/"+y+"' and beigu_datums <= '12/31/"+y+"')) order by pilseta,adrese ")
%><table><%
i = 1
while not r.eof
