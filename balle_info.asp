...
if regions <> "" then
 ...
 if regions <> "K" then
  ...
  "where (((sakuma_dat >= '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and sakuma_dat <'12/31/" & CStr(CLng(Year(Now))) & "') and atcelta <> 1 and veids in (1,2))" & _
  ...
  '"having not sum(balle_vietas) is null"
	...
	ssql = "SELECT IsNull(balle_numurs,b_numurs) as balle_numurs, IsNull(sum(balle_vietas),0) as biletes, " + _  
			...
			"and pieteikums.deleted <> 1 and atcelta <> 1 and sakuma_dat between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/31/" & CStr(CLng(Year(Now))) & "' " + _
  ...
  "(balle_sektors='K' and sakuma_dat between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/31/" & CStr(CLng(Year(Now))) & "') and pieteikums.deleted = 0 " & _
  ...
  "(select piet.id, gid, dbo.IFDNULL(balle_datums,0,balle_vietas) as balle_vietas, balle_datums, balle_knum, sakuma_datums, balle_brivbiletes, piet.deleted from pieteikums piet inner join grupa gr on gr.id = piet.gid where (gid in (292,3100) and sakuma_datums between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/10/" & CStr(CLng(Year(Now))) & "') OR (balle_sektors='K' and sakuma_dat between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/31/" & CStr(CLng(Year(Now))) & "') ) as pieteikums " & _
  "inner join grupa on gid = grupa.id " & _
  "inner join marsruts on mid = marsruts.id " & _
  "inner join grveidi on veids = grveidi.id " & _
  "where (IsNull(valsts,'CITA') in ('CITA') OR (balle_sektors='K' and sakuma_dat between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/31/" & CStr(CLng(Year(Now))) & "') ) and pieteikums.deleted <> 1 " & _
  ...
  "left join grupu_vaditaji gv on gv.idnum = grupa.vaditajs " & _  
  "where gid not in (292,3100) and " & _
  "(balle_sektors='K' and sakuma_dat between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/31/" & CStr(CLng(Year(Now))) & "') and pieteikums.deleted <> 1 " + _
 ...
 '---bileshu skaits ------
	...
	ssql = "SELECT IsNull(balle_numurs,IsNull(b_numurs,0)) as balle_numurs, IsNull(sum(balle_vietas),0) as biletes, " + _  
			...
			"inner join grveidi on veids = grveidi.id " + _ 
			"where gid not in (292,3100) and " & _
			"((balle_sektors='K' or b_sektors='K') and sakuma_dat between '12/10/" & CStr(CLng(Year(Now)) - 1) & "' and '12/31/" & CStr(CLng(Year(Now))) & "') and pieteikums.deleted = 0 " & _
...
ssql = "select p.b_numurs,sum(case when isnull(p.balle_datums,'') <> '' then p.balle_vietas " + _
		"else 0 end) AS datums, sum(isnull(p.balle_vietas,0)) as vietas, sum(isnull(p.balle_brivbiletes,0)) as brivbiletes " + _
...
<form name="forma" action = "balle_info.asp" method = "POST">
...
while not r.eof
 ...
 '---- nedrukaa pelekaas grupas, kur numurs = 0
 if r("balle_numurs") <> 0 then
  ...
  Do While not rNext.EOF
   If r("balle_numurs") <> rNext("balle_numurs") or r("balle_numurs") = 0 then exit do
	...
	if rNext("personas") > 0 or rNext("brivas")> 0 then 
		Response.Write(" <b>" + DatePrint(rNext("sakuma_dat")) + "</b>")
	else
		Response.Write(" "+DatePrint(rNext("sakuma_dat")) )
	end if
...
'--- bloka beigas ----------------------
 ...
 if p1 <> "0" then
	if request.form("regions") <> "K" or p3 <> "0" then
		Response.write p1 & "/" & p2 & " <font color='blue'>(" & p3 & ")"
	else
 ...
 end if '--- nedruka pelekaas grupas, kur numurs = 0
