...
if cp_exist then 
	...
	gv = nullprint(rCurPiet("piezimes"))
else
	gv = ""
end if
...
if pid = 0 then
 set pieteikums = conn.execute ("select NULL as vieta_rezerveta_lidz,"& gid & " as gid,null as summa,null as bilance,'" & SQLDate(now) & "' as datums,0 as students,0 as atcelts,null as piezimes,null as agents,null as carter_firma,null as balle_piezimes, null as balle_vietas, null as balle_datums, null as b_sektors, null as b_numurs, null as ligums_id, null as globa_rez, null as online_rez, null as zid, null as carter_voucer")
else
 set pieteikums = conn.execute ("select vieta_rezerveta_lidz,gid,summa,bilance,datums,students,atcelts,piezimes,agents,carter_firma,balle_vietas,balle_datums,balle_piezimes,dk_norakstita,b_sektors,b_numurs, ligums_id, globa_rez, online_rez, zid, carter_voucer from pieteikums where id = "+cstr(pid))
end if
 ...
 <% if pid<>0 then %>
	...
	if gr_veids = 1 or gr_veids=2 then 'and gid<>rParametri("atteikusies") and gid<>rParametri("atteikusies2") and gid<>rParametri("atcelts") then
		...
		if ligums_id <>0 Then
			...
			</td><td><input type="checkbox" name="ligums_nosutits" <%=nosutits_ch%>>Lîgums nosûtîts parakstîðanai
			</td>
			<% if nosutits then %>
			<td><input type="checkbox" name="ligums_parakstits" <%=parakstits_ch%>><label style="<%=parakstits_style%>">Lîgums parakstîts</label>
			</td>
			<% end if %>
	<%else%>
...
'------------------------------
 ...
 if dk=1 then %>
   ...
   <% 
	...
	%>
	<td colspan="9">Dâvanu karte bûs pieejama pçc iemakas saòemðanas</td>
	<%
   end if
%>
 </tr>
...
for i = 1 to dsk
  ...
  if getnum(dalibnieki("vid"))<>0 then
	  ...
	  %><div id=vid_psid_<%=cstr(dalibnieki("id"))%>><a target='_blank' href="<%if rCurGrupa("carter") then%>vid_select<%else%>vid_select<%end if%>.asp?psid=<%=dalibnieki("id")%>&gid=<%=gid%>&did=<%=did%><%=qs_params%>">Izvçlçties</a></div>
...
set atlaides=server.CreateObject("Adodb.recordset")
	...
	if atlaides.RecordCount>0 then
		...
		while not atlaides.eof
			...
			if trip_start > now or isaccess(T_VECU_PIET_ATLAIDES) then
...
while not iemaksas.EOF
 ...
 End if
	...
	Else
		...
		'response.write "<td><font color="+rowcolor+"><small>"+nullprint(iemaksas("pamatojums"))+"</small></td>"
		noPid = ""
		uzPid = ""
		
		'rw nullprint(iemaksas("NoPid"))
		if nullprint(iemaksas("noPid"))<>0 then
			if  nullprint(iemaksas("noPid")) <> pid then
				
				set rPietGr = conn.execute("select gid from pieteikums where id="+cstr(iemaksas("noPid")))
				pietGid = rPietGr(0)
				grNos = grupas_nosaukums(pietGid,pid)
				noPid = " no <a href='pieteikums_new.asp?pid="+cstr(iemaksas("noPid"))+"' target='_blank' title='"+grNos+" ("+cstr(iemaksas("noPid"))+")'><small>"+left(grNos,35)+"... ("+cstr(iemaksas("noPid"))+")</small></a> "
				
			else
				set rPietGr = conn.execute("select gid from pieteikums where id="+cstr(iemaksas("pid")))
				pietGid = rPietGr(0)
				grNos = grupas_nosaukums(pietGid,pid)
			   uzPid =" uz <a href='pieteikums_new.asp?pid="+cstr(iemaksas("pid"))+"' target='_blank' title='"+grNos+" ("+cstr(iemaksas("noPid"))+")'><small>"+left(grNos,35)+"... ("+cstr(iemaksas("pid"))+")</small></a>"
			   
			end if
		end if
			'response.write "<td><font color="+rowcolor+">"+noPid+"</td>"
		response.write "<td><font color="+rowcolor+">"+nullprint(iemaksas("kas"))+noPid+"</td>"
		response.write "<td><font color="+rowcolor+">"+uzPid+"</td>"
