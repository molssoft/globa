...
dfields = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
	"adrese pilseta indekss talrunisM talrunisD talrunisMob fax eadr nosaukums reg nmkods vaditajs kontaktieris piezimes dzimta rajons novads ids idnr idderdat"
...
dforms = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
	"adrese pilseta indekss talrunisM talrunisD talrunisMob fax eadr nosaukums reg nmkods vaditajs kontaktieris piezimes dzimta rajons novads id_s id_nr id_derdat"
...
<form name="f_add_to_carter" method="POST" action="pieteikums.asp?pid=<%=session("carter_p_id")%>">
	...
	<input type="button" name="del_carter_rez_id" value="Atcelt" onclick="javascript:window.location.assign('dalibn_testam.asp?i=<%=id%>&rez_id=-1');" />
</form>
<%end if '-------------------------------------------------- %>
...
<table border=0>
<!-- @ 0 Forma -------------------------->
<form name="forma" action="dalibn_testam.asp" method="POST" onsubmit="return toUp(this)">
...
<!-- @ 1 Pogas -------------------------->
<input type="image" name="meklet" alt="Atrod dalîbnieku(s) pçc formâ norâdîtajiem datiem. Mekleðana notiek arî pçc daïçji aizpildîta(iem) lauka(iem)." src="impro/bildes/meklet.jpg" onclick="forma.action='dalibn_testam.asp'"><br><br>
<input type="image" alt="No ekranâ redzamâs formas notîra visus datus - sagatavo formu jaunu datu ievadei. Atbilstoðais ieraksts datu bâzç netiek izdzçsts." src="impro/bildes/saktjaunu.jpg" name="saktjaunu" onclick="forma.action='dalibn_testam.asp';">
<input type="image" alt="No ekranâ redzamâs formas notîra visus datus izòemot adresi." src="impro/bildes/saktjaunu2.bmp" name="saktjaunu2" onclick="forma.action='dalibn_testam.asp';">
...
if adrese_var <> "" and pilseta_var <> "" then
  ...
  if not rkaimini.eof then
   ...
   while not rkaimini.eof
    ...
    kstr = "<a href='dalibn_testam.asp?i="+cstr(rkaimini("id"))+"'>" + nullprint(rkaimini("vards"))
 ...
 if pieteikumi("pid")<>p then
	...
	<%
		...
		if getnum(pieteikumi("online_rez"))<>0 then
			...
			%><a href=download_pdf.asp?id=<%=pieteikumi("ligums_id")%> target=ligums><%=pieteikumi("ligums_id")%></a><%
			end if
		end if
