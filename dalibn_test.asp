...
dfields = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
	"adrese pilseta indekss talrunisM talrunisMvalsts talrunisD talrunisDvalsts talrunisMob talrunisMobvalsts fax eadr nosaukums reg nmkods vaditajs kontaktieris piezimes dzimta rajons novads ids idnr idderdat "+_
	"dzimsanas_datums"
...
dforms = "vards uzvards pases pasenr pasederdat pk1 pk2 "+ _
	"adrese pilseta indekss talrunisM talrunisMvalsts talrunisD talrunisDvalsts talrunisMob talrunisMobvalsts fax eadr nosaukums reg nmkods vaditajs kontaktieris piezimes dzimta rajons novads id_s id_nr id_derdat dzimsanas_datums"
...
'rw "select * from dalibn where ID =  " + CStr(id)
	...
	LogUpdateAction "dalibn", id
	id = session("editid")	
	'pârbauda, vai dalîbniekas eksistç arî profils, ja eksistç, atjauno pk , lai nepazûd sasaiste
	'response.write(pk1)
	'response.write("-")
		'response.write(pk2)
		'response.write( "select id from profili where pk1 = '"+cstr(pk1)+"' and pk2 = '"+cstr(pk2)+"'")
	...
	if valid_error <> "" then
		session("dmessage") = "<font color = red><br>"+valid_error+"</font>"
	end if
