...
If request.querystring("balles_biletes") <> "" Then
	conn.execute("update pieteikums set balle_datums = '"+SQLDateYMD(request.form("datums"))+"' where id = " + Request.querystring("pid"))
...
if error = 1 then
	...
	if session("editedcurrency") <> 1 then
		...
		session("qstring_pid") = request.querystring("pid")
...
'Response.end
	...
	valkurs.open "select * from valutakurss where valuta = "+session("valuta")+" and datums = '" + sqldateYMD(session("datums"))+"'",conn,3,3
	...
	Else
		...
		if session("tips") = "izmaksa" then response.redirect("drukord_izm.asp"+qstring())
		if session("tips") = "iemaksa" then response.redirect("drukord.asp"+qstring())
		if session("izm") <> 1 then response.redirect("drukord.asp"+qstring())
