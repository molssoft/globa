...
<tr>
 ...
 <td bgcolor="#fff1cc">
<%Dim valstis: Set valstis = conn.execute("select id, title from valstis where globa = 1 order by dbo.fn_str_to_num(ltrim(title)) asc")
%>
...
<tr>
 <td align="right" bgcolor="#ffc1cc">Vaditajs </td>
 <td bgcolor="#fff1cc">
    <select name=vaditajs>
	<option value="0">-</option>
	<% set rVaditajs = conn.execute("select * from grupu_vaditaji where deleted=0 order by dbo.fn_str_to_num(ltrim(uzvards))")
	while not rVaditajs.eof
	 ...
	 response.write ">"+rVaditajs("uzvards")+" "+rVaditajs("vards")
	 response.write "</Option>"
	 rVaditajs.movenext
	wend
	%></select>
...
<tr>
	...
	<td><select name=kontrolieris>
			  <option value="0">-</option>
			  <% set rKurators = conn.execute("select vards as v, * from lietotaji where exists (select * from tiesibusaites where lietotajsid = lietotaji.id and tiesibasid=21) order by dbo.fn_str_to_num(ltrim(uzvards)) ")
			  while not rKurators.eof
				response.write "<option value="+cstr(rKurators("id"))+" "
...
if Request.Form("valsts") <> "---Visas" then
...
qf="SELECT top 300 g.*,vietas.vietas, m.ID as marID,m.v,m.cena,g.i_cena,m.valsts,kurators,g.autobuss as autobusa_piezimes, "
...
%>
  ...
  <tr bgcolor="#fff1cc" >
    ...
    <td><font color=<%=c%> size="3"><%=f("kods")%>
		<% if f("pdf")<>"" then%>
		<br>
		<font size=1><a href="http://www.impro.lv/pdf/<%=f("pdf")%>.pdf" target="_blank">Ceïojuma apraksts</a></font>
		<% end if%>
	</td>
	...
	<td align=center>
		...
		If (f("lidojums"))<>"" Then
			...
			if not rlidkopa.eof then
				vietas = conn.execute("exec proc_brivas_vietas "+CStr(f("id")))
