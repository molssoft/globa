...
if pid = 0 then 'ja gatava pieteikuma veel nav, ielasam tikai daliibnieka datus
 ...
 "piet_saite.vid, piet_saite.cenaLVL as cenaLVL, piet_saite.cenaUSD as cenaUSD, piet_saite.cenaEUR as cenaEUR, piet_saite.summa as summa, piet_saite.papildv as papildv, piet_saite.vietsk as vietsk, piet_saite.piezimes as piezimes, piet_saite.id as id, piet_saite.kid as kid, "+_
 "piet_saite.vietas_veids as vietas_veids, piet_saite.vietas_cena, piet_saite.vietas_cenaLVL, piet_saite.vietas_cenaUSD,piet_saite.vietas_cenaEUR, piet_saite.kvietas_veids as kvietas_veids,piet_saite.summaLVL,piet_saite.summaUSD,piet_saite.summaEUR  " + _
...
if not isnew then
  if GETNUM(rCurGrupa("veids")) = 6 then kompleks = 1
end if
...
if cp_exist then 
	gv = rCurPiet("piezimes")
else
	gv = ""
end if
	 

if pid <> 0 and gv <> str_gv then %>
...
if pid = 0 then
 set pieteikums = conn.execute ("select "& gid & " as gid,null as summa,null as bilance,'" & SQLDate(now) & "' as datums,0 as students,0 as atcelts,null as piezimes,null as agents,null as carter_firma,null as balle_piezimes, null as balle_vietas, null as balle_datums, null as b_sektors, null as b_numurs, null as ligums_id, null as globa_rez, null as online_rez, null as zid, null as carter_voucer")
else
 set pieteikums = conn.execute ("select gid,summa,bilance,datums,students,atcelts,piezimes,agents,carter_firma,balle_vietas,balle_datums,balle_piezimes,dk_norakstita,b_sektors,b_numurs, ligums_id, globa_rez, online_rez, zid, carter_voucer from pieteikums where id = "+cstr(pid))
end if
...
<%
   ...
   if pid <> 0 then %>
 <form action="piet_save2.asp?pid=<%=pid%>" method="POST" name="forma">
<% else %>
 <form action="piet_save2.asp?pid=<%=pid%>&gid=<%=gid%>&did=<%=did%>" method="POST" name="forma">  
...
if iemsk > 0 or atlsk > 0 then
	...
	if viss = 1 then
		'--- pilns saraksts
	    q_l = "SELECT grupa.kods, grupa.sakuma_dat as sak, marsruts.v, grupa.id "+_
	    "FROM marsruts INNER JOIN grupa ON marsruts.ID = grupa.mID WHERE marsruts.old = 0 ORDER BY dbo.fn_order_string(v), grupa.sakuma_dat;"
	else
		'--- tikai aktualo grupu saraksts
	    q_l = "SELECT grupa.kods, grupa.sakuma_dat as sak, marsruts.v, grupa.id "+_
	    "FROM marsruts INNER JOIN grupa ON marsruts.ID = grupa.mID " + _
	    "WHERE (sakuma_dat >= '" + SQLDate(now-5) + "' OR marsruts.v like '!%' OR marsruts.v like 'Kompleksie%' OR grupa.id = " + cstr(gid) + ") and marsruts.old = 0 " + _
...
<tr>
 <% if pid<>0 then %>
	...
	<td>
	<%
	if gr_veids = 1 then 'and gid<>rParametri("atteikusies") and gid<>rParametri("atteikusies2") and gid<>rParametri("atcelts") then
	
		if isnull(pieteikums("ligums_id")) then 
			...
			if not o_rez.eof Then
				%>
				<a href="download_pdf.asp?id=<%=o_rez("ligums_id")%>" target="_blank">Ceïojuma lîgums Nr. <%=o_rez("ligums_id")%></a>
			<%else%>
				<a href="pieteikumi_grupaa.asp?gid=<%=pieteikums("gid")%>&mode=ligums">Lîgums vairâkiem</a> | 
				<a href="#" onclick="fligums.submit();return false;">Lîgums vienam</a>
			<%end if
		
		else%>
			<a href="download_pdf.asp?id=<%=pieteikums("ligums_id")%>" target="_blank">Ceïojuma lîgums Nr. <%=pieteikums("ligums_id")%></a>
		<%end if
...
<tr>
	...
	<td><input type="text" size="50" name="ppiezimes" value="<%=nullprint(pieteikums("piezimes"))%>">	
</tr>
<%
...
if (gr_veids = 6 or gr_veids = 1 or gr_veids = 2) then %>
 <input type="image" name="saglabat" src="impro/bildes/saglabat.jpg" alt="Tiek saglabâtas izmaiòas, kas izdarîtas pieteikuma teksta laukos un tiek pârrçíinâtas summas un bilance." onclick="return (<%if((gr_veids = 1) and (pid = 0))then Response.Write("checkDalibnInGroup() && checkBlocked() && ")%>CheckKaj() && CheckPakalp() && checkSubmit());">
<% else %>
...
if not isnew then
 ...
 rKajD.open "select kv.nosaukums,did,vards,uzvards,d.nosaukums,kv.vietas,kid from piet_saite ps,kajites_veidi kv,dalibn d, kajite k where kid in (select id from kajite where gid in (" + cstr(gid)+","+cstr(getnum(rCurGrupa("kajites_gid")))+")) and ps.deleted = 0 and veids = kv.id and did=d.id and ps.kid=k.id order by kv.nosaukums,k.id",conn,3,3
...
for i = 1 to dsk
	if nullprint(rCurGrupa("valuta")) <> "XXX" then
	 ...
	 if gr_veids <> 1 then
	  ...
	  response.write("<td align = 'right'><input type='text' size='6' maxlength='20' name='cenaLVL"+cstr(i)+"' value = '"+cstr(getnum(dalibnieki("cenaLVL")))+"'></td>")
	 else
	  response.write("<td align = 'right'><input type='text' size='6' maxlength='20' name='cenaEUR"+cstr(i)+"' value = '"+cstr(getnum(dalibnieki("cenaEUR")))+"'></td>")
	  response.write("<td align = 'right'><input type='text' size='6' maxlength='20' name='cenaUSD"+cstr(i)+"' value = '"+cstr(getnum(dalibnieki("cenaUSD")))+"'></td>")
	  response.write("<td align = 'right'><input type='text' size='6' maxlength='20' name='cenaLVL"+cstr(i)+"' value = '"+cstr(getnum(dalibnieki("cenaLVL")))+"'></td>")
	...
	if not isnew and rKaj.RecordCount > 0 then
		...
		while not rkaj.eof
			...
			if not rKajD.eof then
			 while getnum(rKajD("kid"))=rKaj("id")
			  ...
			  if kaj_aizp <> "" then kaj_aizp = kaj_aizp + ","
			  kaj_aizp = kaj_aizp + rKajD("uzvards")
			  rKajD.MoveNext
			 wend
     ...
     " sum(papild_cena) as papild, sum(papild2_cena) as papild2 " + _
     " from kajites_veidi where gid = "+cstr(gid))
			...
			<option value = "2" <% if x = 2 then Response.Write "selected" %>>Bçrnu</option>
			<% end if %>
			
			<% if getnum(KajCenas("pusaudzu"))>0 then %>
			<option value = "5" <% if x = 5 then Response.Write "selected" %>>Pusaudþu</option>
			<% end if %>
  ...
  if getnum(dalibnieki("vid"))<>0 then
	   ...
	   if v = "" then v = "???" 
	   Response.Write "<div id=vid_psid_"+nullprint(dalibnieki("id"))+"><a target=vid_select href="""
	   ''if rCurGrupa("carter") then 
		''	rw "vid_select_carter"
	   ''else
		''	rw "vid_select"	   
	   ''end if
	   'Response.Write ".asp?psid="+cstr(dalibnieki("id"))+qs_params+""">"+v+"</a></div>"
	   Response.Write "vid_select.asp?psid="+cstr(dalibnieki("id"))+""">"+v+"</a></div>"
	   'Viesnicas_combo "vid"+cstr(i),gid,dalibnieki("vid")
	 else
	
	  %><div id=vid_psid_<%=cstr(dalibnieki("id"))%>><a target=vid_select href="<%if rCurGrupa("carter") then%>vid_select<%else%>vid_select<%end if%>.asp?psid=<%=dalibnieki("id")%>&gid=<%=gid%>&did=<%=did%><%=qs_params%>">Izvçlçties</a></div>
	...
	if gv = str_gv then 'parbaude esosham pieteikumam. grupas vaditajam tikai viens pak - grupas nauda
...
set atlaides=server.CreateObject("Adodb.recordset")
	...
	if atlaides.RecordCount>0 then
		...
		while not atlaides.eof
			...
			Response.Write "id="+cstr(atlaides("id"))+"');"+chr(34)+"></td></tr>"
			atlaides.MoveNext
		wend 
		%>
		<% if pid<>0 then %>
