...
table = "valstis"
order = "id"
captions = "Identifikators,Valsts"
captions_mas = split(captions,",")
fields = "Nosaukums,Vieta,Atstats,Panemts"
fields_mas = split(fields,",")
types = "varchar,varchar,date,bit"
...
if r.recordcount <> 0 then
   ...
   next 
   %>
		 <td><input type="image" src="impro/bildes/dzest.jpg" onclick="forma.id.value=<%=cstr(r("id"))%>;forma.act.value='delete';return confirm('Vai dz�st ierakstu?');" alt="Dz�st ierakstu."></td>
		 <td><input type="image" src="impro/bildes/diskete.jpg" onclick="forma.id.value=<%=cstr(r("id"))%>;forma.act.value='update'" alt="Saglab�t ierakstu."></td></tr>
		<%
		r.MoveNext
	wend
end if
