...
query ="select isnull(orez.tika_lidz_bankai,0) as tika_lidz_bankai,orez.deleted,sakuma_dat,orez.id, max(p.datums) as datums, g.kods, m.v, pr.vards, pr.uzvards, count(p.personas) as dalibnieki, " + _ 
...
"where ((orez.deleted = 0 and p.deleted = 0 and p.tmp = 0 and (p.step = '4' or orez.no_delete=1)) or (orez.deleted=1 and isnull(orez.tika_lidz_bankai,0)=1)) and p.internets = 1 " + whereC + _ 
"group by orez.tika_lidz_bankai, orez.deleted,sakuma_dat, orez.id, g.kods, m.v, orez.ligums_id, pr.vards, pr.uzvards, p.atlaidesLVL " + _
...
<table class="form_table">
	...
	<% While Not result.eof 
			...
			'CurrPrint(summaLVL)
			style = ""
		if result("deleted")="1" and result("tika_lidz_bankai") then style="style='color:red'" end if
		%>
			...
			<tr >
				...
				<td <%=style%>><a href="user_tracking_new.asp?rez_id=<%=result("id")%>" ><%Response.write(result("vards")&" "&result("uzvards"))%></a></td>
