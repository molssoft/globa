...
//21.08.2019 RT:
function sendData(){
	var params = $('input[name="vairaki_vv[]"]').serialize();
	//console.log(params.length);
	if (params.length == 0){
		alert('Lûdzu, ieíesçjiet vismaz vienu pakalpojuma veidu kolonnâ  "Pasûtîts"!');
	}
	else {
	
		window.open("c_vietu_veidi.php?gid=<%=cstr(gid)%>&"+params,'_blank');
	}
	 return false;
...
if Not r.eof then
	...
	<th>Max vecums</th>
	<th>Papild v.</th>
	<th>Nerâdît<br>online</th>
	<th>Visa<br>istaba</th>
	<th>Klienta<br>cena</th>
	...
	while not r.eof
		...
		Set rspec = conn.execute("Select laiks_no, laiks_lidz, spec_id,viesnicas_veids,vietu_veidi.id,vietu_veidi.nosaukums,vietu_veidi.tips,vietu_veidi.max_vecums,vietu_veidi.min_vecums,cena,cenaEUR,cenaUSD,limits,limita_grupa,persona,papildv,no_online,viesnicas_veidi.nosaukums as vnosaukums,carter_berns_num,ekskursija from vietu_veidi left join viesnicas_veidi on vietu_veidi.viesnicas_veids = viesnicas_veidi.id where vietu_veidi.spec_id = " + cstr(r("id")) + " order by vietu_veidi.nosaukums")
	...
	%>
		<tr>
			<td colspan="15"></td>
			<td><a   href="#" name="vairaki_vv" value="1" onclick="sendData();">Parâdît<br>sarakstu</a></td>
			<td></td>
		</tr>
	</table>
	<% if gid<>"" then %>
...
%><br><a target = none href = "vietu_veidi_vesture.asp?gid=<%=gid%>"><img border = 0 src="impro/bildes/clock.bmp" alt="Apskatît vçsturi."></a>
...
<% for i = 1 to 5 %>
	...
	<% else %>
		...
		<td align="center"><input type="text" name="cenaUSD<%=i%>" value=0 size="4" disabled></td>
	<% end if %>
...
Sub PrintRow(r)
		set r2 = conn.execute("select * from vietu_veidi where id = "+cstr(r("id")))

		bg = "#fff1cc"
		If getnum(r("spec_id"))<>0 Then bg = "88FF88"
		
		%>
		<tr bgcolor = <%=bg%>><td><input type = text name = "nosaukums<%=r("id")%>" size = 50 value = "<%=NullPrint(r("nosaukums"))%>">
		<%
		...
		<% else %>
			...
			<td align = right><input type = text name = "cenaUSD<%=r("id")%>" size = 4 value = <%=GetNum(r("cenaUSD"))%> disabled></td>
		<% end if %>
<!-- viesnicas iznjemts 26 feb 2009 /Nils-->
	...
	<!--	<td align = right><input type = text name = "limita_grupa<%=r("id")%>" size = 10 value = <%=NullPrint(r("limita_grupa"))%>></td>-->
		...
		<td align = right><input type = text name = "min_vec_<%=r("id")%>" size=2 maxlength=2 value=<%=r("min_vecums")%>></td>
		<% else %>
		<td align = right>&nbsp;</td>
		<% end if %>
		...
		<td align = center><input type = checkbox name = "no_online<%=r("id")%>" <% if r2("no_online") = true then Response.Write " checked "%>></td>
		<td align = center><input type = checkbox name = "visa_istaba<%=r("id")%>" <% if r2("visa_istaba") = true then Response.Write " checked "%>></td>
		<td align = center><input type = checkbox name = "klienta_cena<%=r("id")%>" <% if r2("klienta_cena") = true then Response.Write " checked "%>></td>
		<%if rGrupa("carter") then%>
		<!--td align = center-->
		...
		<td align = right><%
		set rPasVietas = conn.execute("select sum(vietsk) from piet_saite where deleted = 0  and pid in (select id from pieteikums where deleted = 0 and id = piet_saite.pid) and vietas_veids="+cstr(r("id")))
		if not rPasVietas.eof then
			if rPasVietas(0) <> 0 then
			%>
			<input type="checkbox" title="Iekïaut sarakstâ" name="vairaki_vv[]" value="<%=cstr(r("id"))%>" form="pakalpojumu_sar_forma">
			<a href="c_vietu_veidi.php?gid=<%=cstr(gid)%>&vv=<%=cstr(r("id"))%>" target="_blank"><%=rPasVietas(0)%></a>
			<%
			end if 
		end if%>
		</td>
